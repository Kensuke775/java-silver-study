...
- **フィールド（`this.y`）**：オブジェクト自身が持つデータ本体。コンポーネントの実体であり、**代入される側（代入先）**
- **パラメータ／引数（`y`、`this.`が付いていない方）**：コンストラクタ呼び出し時に渡された一時的な値。**代入する側（代入元）**

`this.`が付いているかどうかで「オブジェクト本体のデータ（フィールド）」か「今受け取ったばかりの値（パラメータ）」かを区別する。

### static vs インスタンス（再確認）

recordに`static`フィールド・`static`メソッドは定義できるが、コンポーネント以外の**インスタンス**フィールドは持てない。理由は、recordの目的が「不変なデータの塊」であることで、`static`はインスタンスの状態ではなく**クラス全体で共有される1つの値**なので、この不変性の制約とは無関係だから禁止されない。禁止されているのはあくまで「インスタンスごとに変わりうる、コンポーネント外の状態」だけ。

### recordの暗黙の修飾子

| record内の要素 | 暗黙で付く修飾子 |
|---|---|
| コンポーネントの裏側にあるフィールド | `private final` |
| アクセサメソッド（`x()`, `y()`など） | `public`のみ（`static`でも`final`でもない） |

フィールドが`private final`＝外部から直接触れず、しかも再代入不可（不変）。アクセサメソッドが`public`のみ＝外部から値を読み取る手段は公開されているが、`static`でも`final`でもないので普通にオーバーライドも可能（問題1-Eで確認済み）。「データは隠して守る（private final）、読み取り口だけは公開する（public accessor）」という、カプセル化の典型的な形がrecordには自動で組み込まれている。

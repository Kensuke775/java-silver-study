...
## 応用問題6-8：this()委譲とフィールド初期化・インスタンス初期化ブロックの順序
...
メソッド名も`static`も付かず、クラス直下に書かれた`{ }`だけのブロック。**インスタンスが生成されるたびに**実行される（`static { ... }`＝クラスロード時に1回だけ、とは別物なので混同注意）。

**フィールド初期化子とインスタンス初期化ブロックは、書かれている順番通りに上から交互に実行される。** 今回はフィールド初期化子が先に書かれていたので「"field x" → "instance initializer block"」の順になった。

### 実行タイミングの全体像

フィールド初期化・インスタンス初期化ブロックが実行されるのは「**`super()`が完全に終わった直後、コンストラクタ自身の本体が始まる前**」。これは`this(...)`で委譲していても変わらず、**最終的にsuper()を呼ぶことになるコンストラクタでだけ1回**実行される（委譲元・委譲先の両方で二重に実行されたりはしない）。

```
new A() 開始
  ↓
A() の1行目 this(100); に到達 → A(int v) へ処理がまるごと移る（A()は一時停止）
  ↓
A(int v) の1行目：明示的なthis()/super()がないので、コンパイラが自動挿入したsuper()（Objectの空コンストラクタ）が動く
  ↓
super()が終わった直後、A(int v)自身の本体が始まる前に、
フィールド初期化・インスタンス初期化ブロックが順番に実行される
  → "field x" → "instance initializer block"
  ↓
A(int v) 自身の本体が実行される → "A(int) v=100"
  ↓
A(int v) が完全に終わったので、②で一時停止していたA()に戻る
  ↓
A() の続きの行が実行される → "A()"
```

**`this(...)`/`super(...)`は普通のメソッド呼び出しと同じ**：呼ぶ→相手の処理が全部終わる→呼んだ場所の続きに戻ってきて上から順に読み進める、という基本原則がここでも成り立つ。「最後に実行される」のではなく「呼ばれた瞬間に割り込んで、完全に終わるまで元の場所の続きは実行されない」という動き。

## 応用問題6-9：recordのコンストラクタ・メンバールールを総整理

### recordヘッダーの括弧は「コンポーネント0個でも」省略不可

```java
record Empty() {}   // ○ 合法（コンポーネント0個でも括弧は必須）
record Empty {}      // ✗ エラー：「レコード・ヘッダーが必要です」
```

（javacで実機検証済み）「中身が0個かどうか」ではなく「括弧という構文要素自体があるかどうか」が合否を分ける。

### コンストラクタは3パターンあり、`this.x=x`の要不要が全部違う

| 書き方 | 例 | `this.x=x`等は書けるか |
|---|---|---|
| コンストラクタを書かない | `record Point(int x, int y) {}` | 不要（丸ごと自動生成） |
| コンパクトコンストラクタ（引数リストなし） | `Point { x = Math.abs(x); }` | **書くと逆にエラー**。パラメータ`x`の書き換えは合法だが`this.x=x`のようなフィールドへの直接代入は禁止 |
| 標準（長形式）の正準コンストラクタ（引数リストあり） | `Point(int x, int y) { this.x=x; this.y=y; }` | **必須**。書かないと「変数xは初期化されていない可能性があります」エラー |

（javacで実機検証済み。コンパクトコンストラクタで`this.x=x`を書くと「final変数xに値を代入することはできません」エラーになることを確認）

**なぜコンパクトコンストラクタで`this.x=x`が禁止されるのか**：コンパクトコンストラクタは「あなたが書いたコードの後に、コンパイラが自動で`this.x=x; this.y=y;`を追記する」という仕組み。先に自分で書いてしまうとfinalフィールドへの二重代入になるため禁止される。コンパクトコンストラクタで触れるのは「まだフィールドに代入される前のパラメータ`x`・`y`そのもの」だけ。

### 非正準コンストラクタは`this(...)`委譲が必須

```java
// ✗ エラー：コンストラクタが標準でないため、先頭文で他のコンストラクタを呼ぶ必要がある
Point(int y) {
    this.y = y;
    this.x = 0;
}

// ○ 合法：this(...)で正準コンストラクタに委譲している
Point(int y) {
    this(0, y);   // 1番目の引数0→x、2番目の引数y→y（名前ではなく位置で対応）
}
```

6-8応用の「非正準コンストラクタはthis(...)委譲必須」というルールがrecordにもそのまま適用される。

### recordに追加できるメンバー・できないメンバー

| 追加できるもの | 追加できないもの |
|---|---|
| `static`フィールド | コンポーネント以外の**インスタンス**フィールド（初期値の有無に関係なく不可） |
| `static`メソッド | ー |
| アクセサメソッドのオーバーライド（`public int x() { ... }`） | ー |
| ネストしたrecord（暗黙的に`static`） | ー |
| **ローカルrecord**（メソッドの中で定義。Java 16+、暗黙的にstatic） | ー |

（javacで実機検証済み：static フィールド/メソッド、アクセサのオーバーライド、ネストrecord、ローカルrecordすべてコンパイル成功。コンポーネント外のインスタンスフィールドはinitializerの有無に関わらずエラー）

### record vs interfaceの「暗黙の修飾子」を混同しない

| | 暗黙で付く修飾子 |
|---|---|
| **interfaceのフィールド**（定数） | `public static final` |
| **recordのアクセサメソッド**（`x()`など） | `public`のみ（static/finalではない） |
| recordのコンポーネントの裏フィールド | `private final` |
| recordクラス自体 | `final` |

recordのアクセサメソッドが`static`でも`final`でもないからこそ、普通にオーバーライド可能（もし`static`や`final`が付くならオーバーライド自体が成立しない）。「recordだからinterfaceと同じくpublic static finalが付く」という思い込みは誤り。
